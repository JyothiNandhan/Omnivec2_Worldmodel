...
from s2_world_model_2.temporal_dataset import build_world_model_dataloaders  # noqa: E402
from s2_world_model_2.world_model_s2 import (  # noqa: E402
...
def parse_args():
    ...
    p.add_argument("--output_dir",        type=str,   default="./runs/s2_world_model_2")
    ...
    p.add_argument("--temporal_inner_dim",type=int,   default=128,
                   help="Internal transformer dim for TemporalTokenPredictor. "
                        "WM3 overrides this to 256 for greater capacity.")
    ...
    p.add_argument("--contrastive_weight", type=float, default=None,
                   help="World Model 3 only: weight for future-vs-present contrastive loss.")
    p.add_argument("--rgb_patch_weight", type=float, default=1.0,
                   help="Weight for direct future RGB patch prediction loss.")
    p.add_argument("--ssim_weight", type=float, default=0.5,
                   help="World Model 3 only: weight for SSIM loss on decoded future image.")
    p.add_argument("--lidar_weight", type=float, default=1.0,
                   help="Scale factor for LiDAR token MSE loss. Reduce (<1.0) if LiDAR converges faster than RGB.")
    p.add_argument("--variance_weight", type=float, default=1.0,
                   help="Weight for VICReg-style variance loss that prevents token collapse.")
    args = p.parse_args()
...
def visualize(model, val_dl, args):
    ...
    # Prefer the direct future-patch head when available. It is trained against
    # real future RGB patches, unlike the Stage 2 masked decoder path.
    if "pred_rgb_patches" in out:
        pred_patches = out["pred_rgb_patches"]
    else:
    ...
    # Direct future RGB prediction, or the legacy token-decoder fallback.
    raw_pred_imgs = unpatchify(pred_patches)
    if "pred_rgb_patches" not in out:
        raw_pred_imgs = (raw_pred_imgs + 2.0) / 4.0  # Scale normalized patches to [0, 1]

    # 2. COLORIZED PREDICTION (uses predicted color statistics for the old
    # decoder path; direct patch prediction is already in pixel space).
    if "pred_rgb_patches" in out:
        color_pred_imgs = raw_pred_imgs
    else:
    ...
    change_maps = (raw_pred_imgs - rgb_seq[:, -1]).abs().mean(dim=1)

    model_rgb_mae = F.l1_loss(raw_pred_imgs, rgb_tgt).item()
    base_rgb_mae = F.l1_loss(rgb_seq[:, -1], rgb_tgt).item()
    model_rgb_pixel_mse = F.mse_loss(raw_pred_imgs, rgb_tgt).item()
    base_rgb_pixel_mse = F.mse_loss(rgb_seq[:, -1], rgb_tgt).item()

    # ── 1. Context strip with Model Prediction ───────────────────────────────
    for i in range(n):
        # History (T), predicted future, learned change map, ground truth.
        ...
        # Plot direct future prediction
        raw_img = raw_pred_imgs[i].permute(1,2,0).cpu().numpy()
        axes[-3].imshow(np.clip(raw_img, 0, 1))
        axes[-3].set_title(
            f"MODEL RGB T+{pred_frame_label}\n"
            f"({time_ahead_s:.1f}s future prediction)",
        ...
        # Show where the residual predictor changed the last observed frame.
        change_img = change_maps[i].cpu().numpy()
        axes[-2].imshow(change_img, cmap="magma", vmin=0.0, vmax=max(0.1, float(change_img.max())))
        axes[-2].set_title(
            "PREDICTED CHANGE FROM T\n"
    ...
    # ── 1b. Pixel-level comparison strip: copy-last | model | ground truth ───
    for i in range(n):
        fig, axes = plt.subplots(1, 3, figsize=(12, 3.6))
        last_img = rgb_seq[i, -1].permute(1,2,0).cpu().numpy()
        model_img = raw_pred_imgs[i].permute(1,2,0).cpu().numpy()
        gt_img    = rgb_tgt[i].permute(1,2,0).cpu().numpy()

        axes[0].imshow(np.clip(last_img, 0, 1))
        axes[0].set_title("Copy-Last Baseline (T)", fontsize=9, color="#7d3c98", fontweight="bold")
        axes[0].axis("off")
        for sp in axes[0].spines.values():
            sp.set_edgecolor("#7d3c98"); sp.set_linewidth(3)

        axes[1].imshow(np.clip(model_img, 0, 1))
        axes[1].set_title(
            f"Model Prediction T+{args.steps_ahead} ({args.steps_ahead*0.5:.1f}s)",
            fontsize=9, color="#2878b5", fontweight="bold")
        axes[1].axis("off")
        ...
        axes[2].set_title(
            f"Ground Truth T+{args.steps_ahead} (reference)",
            fontsize=9, color="darkred", fontweight="bold")
        axes[2].axis("off")
        for sp in axes[2].spines.values():
            sp.set_edgecolor("red"); sp.set_linewidth(3)

        fig.suptitle(f"Sample {i} — Pixel Comparison: Baseline vs Model vs Ground Truth")
        fig.tight_layout()
        fig.savefig(os.path.join(vd, f"s2_wm_pixel_comparison_{i}.png"),
                    dpi=160, bbox_inches="tight"); plt.close(fig)
    print("[VIS] Pixel comparison strips saved.")

    # ── 4. Cosine histograms: Baseline (red) vs Model (blue) ─────────────────
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    bins = np.linspace(-0.2, 1.01, 62)
    ...
    with open(rpt, "w") as f:
        ...
        f.write("  RGB output    : residual future-patch prediction over the last observed image\n\n")
        ...
        for name, bv, mv, lb in [
            ...
            ("RGB pixel MAE (lower=better)",   base_rgb_mae, model_rgb_mae, True),
            ("RGB pixel MSE (lower=better)",   base_rgb_pixel_mse, model_rgb_pixel_mse, True),
        ]:
            imp = pct(mv, bv, lb)
        ...
        f.write("  s2_wm_gt_context_strip_*.png         - history + model pred + GT strip\n")
        f.write("  s2_wm_pixel_comparison_*.png         - copy-last | model | GT side-by-side\n")
        f.write("  s2_wm_model_vs_baseline_heatmap.png  - per-patch token MSE comparison\n")
...
def main():
    ...
    temporal = TemporalTokenPredictor(
        embed_dim=FC_HIDDEN,
        inner_dim=args.temporal_inner_dim,
